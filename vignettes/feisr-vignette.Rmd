...
The functions included in the R package `feisr` are also available in the [xtfeis ado](https://www.stata.com/meeting/germany10/germany10_ludwig.pdf) for Stata. The package [plm](https://CRAN.R-project.org/package=plm) provides esimation of related models, as for instance the mean group (MG) or common correlated effects mean groups (CCEMG) estimator via `pmg()` function and the estimator of models with variable coefficients via `pvcm()`.
...
To illustrate the usage of the `feisr` package, we exemplary investigate the 'marriage wage premium': we analyze whether marriage leads to an increase in the hourly wage for men. We use the function `feis` to estimate fixed effects individual slope models to control for the hypothesis that those men who marry earlier also have a steeper wage growth over time. Similar to the  `plm` function, `feis` requires to indicate a unique person / group identifier. To include individual-specific slopes, `feis` uses two-part formulas `(expr | slope_expr)`, where `slope_expr` gives the expression for modelling the individual slopes. 
...
The fact that we observe an overestimation of the effect in the conventional FE model and receive a highly significant value in the specification tests indicates that the data exhibit a positive covariance between the effect of experience on wages and the effect of experience on marriage: $\Cov(\ddot{\bm \delta},\ddot{\bm \alpha}_2)> 0$. To directly test this hypothesis we can extract the individual slope parameters $\bm \alpha_{i2}$ using the `slopes()` function and compute the $\bm \delta_i$ vectors in a loop of individual regressions:
```{r slope, width = 70}
alpha2 <- data.frame(slopes(wages.feis))

delta <- alpha2
delta[, 1:ncol(delta)] <- NA

for(i in rownames(alpha2)){
  tmp <- mwp[mwp$id == i, ]
  lm <- lm(lnw ~ exp + I(exp^2), data = tmp)
  delta[i, ] <- t(lm$coefficients)
}

cov(alpha2$exp, delta$exp)
cov(alpha2$I.exp.2., delta$I.exp.2.)
```

This confirms that those individuals with a steeper slope in marriage also have a steeper slope in wages: men with a stronger wage growth marry earlier. This positive covariance leads to an upwardly biased point estimate in conventional FE and RE models. In more detail, this is shown analytically and by Monte Carlo simulations in @Ruttenauer.2020.
