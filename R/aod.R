### This is a copy of the test functions in the
### aod package by Matthieu Lesnoff and Renaud Lancelot
### https://cran.r-project.org/package=aod
### Copied to avoid archival
...
wald.test <- function(Sigma, b, Terms = NULL, L = NULL, H0 = NULL, df = NULL, verbose = FALSE){
  ...
  if(is.null(df))
    ...
    df1 <- nrow(L); df2 <- df
    res <- list(chi2 = c(chi2 = stat, df = w, P = p),
                Ftest = c(Fstat = fstat, df1 = df1, df2 = df2, P = 1 - stats::pf(fstat, df1, df2)))
  }
